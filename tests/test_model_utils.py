...
from src.spaceship_titanic.model_utils import (
    evaluate_model,
...
def test_evaluate_model_target_recall(mock_model, mock_data):
    ...
    result = evaluate_model(
        mock_model, x_test, y_test, target_recall=target_recall
    )
...
def test_extract_feature_importances_random_forest(mock_model, mock_data):
    ...
    importances = extract_feature_importances(
        mock_model, pd.DataFrame(x_train), y_train
    )
...
def test_extract_feature_importances_non_feature_importance_model(mock_data):
    """Test extract_feature_importances with a model that doesn't have
    ...
    importances = extract_feature_importances(
        dummy_model, pd.DataFrame(x_train), y_train
    )
